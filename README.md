...
| Deployed to a non-`us-east-1` region and see no logs / metrics / traces in that region's CloudWatch, and the `.ebextensions` log group is not auto-created | App defaulted to `us-east-1` and shipped everything there. `botocore.Session().get_config_variable("region")` does NOT read from IMDS, and `AWS_REGION` isn't set in the EB env by default. The `.ebextensions` `commands:` block had the same bug via `${AWS_REGION:-us-east-1}` | Fixed in this repo — `application.py` now uses `InstanceMetadataRegionFetcher().retrieve_region()` explicitly, and `.ebextensions/02-cloudwatch-permissions.config` drops the `--region` flag so the CLI uses IMDS. Redeploy with `eb deploy` after pulling. To confirm you were hitting this, check for recent activity in the us-east-1 log group: `aws logs describe-log-streams --log-group-name /otel/demo/direct-logs --region us-east-1 --order-by LastEventTime --descending` |
